...
namespace PariwisataApp
{
	public class DetailFragment : Android.Support.V4.App.Fragment
	{
		...
		{
			View view = inflater.Inflate(Resource.Layout.detailLayout, container, false);
			...
			var jsonstring = client.DownloadString("https://banyu.center/pariwisataapp/api/getWisataDetail/" + pId);
			JObject o = JObject.Parse(jsonstring);
			var ab = o.GetValue("detail_wisata").ToString();
			JArray arr = JArray.Parse(ab);
			//idnya.Text = arr[0]["id"].ToString();
			NamaDetail.Text = arr[0]["nama_wisata"].ToString();
			txtKeterangan.Text = arr[0]["keterangan"].ToString();
			HargaDetail.Text = arr[0]["harga_tiket"].ToString();
			...
			string geo = arr[0]["nama_wisata"].ToString();
